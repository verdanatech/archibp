...
-- -----------------------------------------------------
SELECT `id` INTO @table_id FROM `glpi_plugin_statecheck_tables` WHERE `name` = 'glpi_plugin_archisw_configbpfieldgroups';
SELECT `id` INTO @rule_id FROM `glpi_plugin_statecheck_rules` WHERE `plugin_statecheck_tables_id` = @table_id AND `name` = 'Business Process Field Groups - mandatory fields' AND `plugin_statecheck_targetstates_id` = 0;
